...
struct BlifDumper
{
	...
	BlifDumper(std::vector<std::string> &f, RTLIL::Module *module, RTLIL::Design *design, BlifDumperConfig *config) :
	...
	void dump_params(const char *command, dict<IdString, Const> &params)
	{
		...
		for (auto &param : params) {
			buf = stringf("%s %s ", command, log_id(param.first));
	...
	void dump()
	{
		...
		for (auto &it : inputs) {
			...
			for (int i = 0; i < wire->width; i++)
				buf += stringf(" %s", str(RTLIL::SigSpec(wire, i)).c_str());
		}
		buf += stringf("\n");
		f.push_back(buf);
		buf.clear();
		...
		if (module->get_blackbox_attribute()) {
			f.push_back(stringf(".blackbox\n"));
			f.push_back(stringf(".end\n"));
			return;
		}
		...
		for (auto cell : module->cells())
		{
			...
			if (!config->icells_mode && cell->type == ID($_XOR_)) {
				f.push_back(stringf(".names %s %s %s\n",
						str(cell->getPort(ID::A)).c_str(), str(cell->getPort(ID::B)).c_str(), str(cell->getPort(ID::Y)).c_str()));
				f.push_back(stringf("10 1\n"));
				f.push_back(stringf("01 1\n"));
				goto internal_cell;
			}
			...
			if (!config->icells_mode && cell->type == ID($_AOI4_)) {
				f.push_back(stringf(".names %s %s %s %s %s\n",
			...
			if (!config->icells_mode && cell->type == ID($lut)) {
				...
				for (int i = width-1; i >= 0; i--)
					buf += stringf(" %s", str(inputs.extract(i, 1)).c_str());
				auto &output = cell->getPort(ID::Y);
				log_assert(output.size() == 1);
				buf += stringf(" %s", str(output).c_str());
				buf += stringf("\n");
				f.push_back(buf);
				buf.clear();
			...
			buf += stringf(".%s %s", subckt_or_gate(cell->type.str()), str(cell->type).c_str());
			for (auto &conn : cell->connections())
			{
				if (conn.second.size() == 1) {
					buf += stringf(" %s=%s", str(conn.first).c_str(), str(conn.second[0]).c_str());
					continue;
				}
				...
				if (w == nullptr) {
					for (int i = 0; i < GetSize(conn.second); i++)
						buf += stringf(" %s[%d]=%s", str(conn.first).c_str(), i, str(conn.second[i]).c_str());
				} else {
					for (int i = 0; i < std::min(GetSize(conn.second), GetSize(w)); i++) {
						SigBit sig(w, i);
						buf += stringf(" %s[%d]=%s", str(conn.first).c_str(), sig.wire->upto ?
	...
	static void dump(std::vector<std::string> &f, RTLIL::Module *module, RTLIL::Design *design, BlifDumperConfig &config)
	{
...
struct BlifUtil {
	void execute(std::vector<std::string> *&f, std::string filename, std::vector<std::string> args, RTLIL::Design *design)
	{
		...
		(*f).push_back(stringf("# Generated by %s\n", yosys_version_str));
